{$view_context = CerberusContexts::CONTEXT_SERVER}
...
<table cellpadding="0" cellspacing="0" border="0" class="worklist" width="100%">
	<tr>
		...
		<td nowrap="nowrap" align="right" class="title-toolbar">
			<a href="javascript:;" title="{'common.add'|devblocks_translate|capitalize}" class="minimal cerb-peek-trigger" data-context="{$view_context}" data-context-id="0" data-edit="true"><span class="glyphicons glyphicons-circle-plus"></span></a>
...
<table cellpadding="1" cellspacing="0" border="0" width="100%" class="worklistBody">
	...
	{* Column Data *}
	{$object_watchers = DAO_ContextLink::getContextLinks('cerberusweb.contexts.datacenter.server', array_keys($data), CerberusContexts::CONTEXT_WORKER)}
	...
	<tbody style="cursor:pointer;">
		<tr class="{$tableRowClass}">
			<td data-column="*_watchers" align="center" rowspan="2" nowrap="nowrap" style="padding:5px;">
				{include file="devblocks:cerberusweb.core::internal/watchers/context_follow_button.tpl" context='cerberusweb.contexts.datacenter.server' context_id=$result.s_id}
			</td>
			<td data-column="label" colspan="{$smarty.foreach.headers.total}">
				...
				<a href="{devblocks_url}c=profiles&type=server&id={$result.s_id}-{$result.s_name|devblocks_permalink}{/devblocks_url}" class="subject">{$result.s_name}</a>
				<button type="button" class="peek cerb-peek-trigger" data-context="{$view_context}" data-context-id="{$result.s_id}"><span class="glyphicons glyphicons-new-window-alt"></span></button>
			</td>
		</tr>
		<tr class="{$tableRowClass}">
...
<div style="padding-top:5px;">
	...
	<div style="float:left;" id="{$view->id}_actions">
		<button type="button" class="action-always-show action-explore" onclick="this.form.explore_from.value=$(this).closest('form').find('tbody input:checkbox:checked:first').val();this.form.a.value='viewServersExplore';this.form.submit();"><span class="glyphicons glyphicons-play-button"></span> {'common.explore'|devblocks_translate|lower}</button>
		<button type="button" class="action-always-show action-bulkupdate" onclick="genericAjaxPopup('peek','c=profiles&a=handleSectionAction&section=server&action=showBulkPopup&view_id={$view->id}&ids=' + Devblocks.getFormEnabledCheckboxValues('viewForm{$view->id}','row_id[]'),null,false,'50%');"><span class="glyphicons glyphicons-folder-closed"></span> {'common.bulk_update'|devblocks_translate|lower}</button>
	</div>
</div>
